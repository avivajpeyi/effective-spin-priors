...
def plot_cos_given_other_params(q, a1, a2, xeff):
    ...
    valid_cos2 = [
        cos_lims[0] <= cos_2_vals[i] <= cos_lims[1] for i in range(len(cos_2_vals))
    ]
...
def filter_invalid_cos2(s):
    ...
    s["valid_cos2"] = (lower <= s["cos_tilt_2"]) & (s["cos_tilt_2"] <= upper)
...
def filter_invalid_xeff(s):
    s["valid_xeff"] = (s["a_1"] + s["q"] * s["a_2"]) / (s["q"] + 1) >= np.abs(s["xeff"])
...
def get_valid_xeff_samples(q, a1, a2):
    ...
    p_xeff = chi_effective_prior_from_isotropic_spins(
        q=q, aMax=np.maximum(a1, a2), xs=xeffs
    )
...
def get_samples(num=10):
    ...
    s["cos_tilt_2"] = get_valid_cos2_samples(
        a1=s.a_1, a2=s.a_2, q=s.q, xeff=s.xeff, cos_prior=cos_prior
    )
    s["cos_tilt_1"] = calc_cos1(
        q=s.q, a1=s.a_1, a2=s.a_2, xeff=s.xeff, cos2=s.cos_tilt_2
    )
...
def plot_samples(s, param, sample_ranges=None):
    ...
    r = bilby.core.result.Result(
        search_parameter_keys=p, parameter_labels=p, posterior=s
    )
    fig = r.plot_corner(
        parameters=param, priors=get_prior_with_xeff(), range=sample_ranges
    )
    return fig
...
def rory_p_xeff_given_a1_a2_q(chi_eff, a1, a2, q):
    ...
    if (chi_eff < chi_eff_min) or (chi_eff_max < chi_eff):
        ...
        return (
            ...
            / (4 * a1 * a2 * q)
            * (
                np.minimum(a1 / (1 + q), chi_eff + a2 * q / (1 + q))
...
def plot_xeff_given_a1_a2_q(a1, a2, q):
    ...
    samps = calculate_xeff(
        a1=a1, a2=a2, cos1=unip.sample(10000), cos2=unip.sample(10000), q=q
    )
    fig, ax = plt.subplots()
